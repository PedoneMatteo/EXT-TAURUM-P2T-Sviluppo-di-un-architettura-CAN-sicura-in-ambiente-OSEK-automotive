...
//#ifdef BACK_TO_TABLES /* pre-calculated tables
...
static void KeyExpansion(uint8_t* RoundKey, const uint8_t* Key)
{
  ...
  for (i = Nk; i < Nb * (Nr + 1); ++i)
  {
    ...
    if (i % Nk == 0)
    {
      ...
      // Function Subword()
      {
        tempa[0] = rj_sbox(tempa[0]);
        tempa[1] = rj_sbox(tempa[1]);
        tempa[2] = rj_sbox(tempa[2]);
        tempa[3] = rj_sbox(tempa[3]);
      }
...
void AES_init_ctx_iv(aes256_context * ctx, uint8_t* key, uint8_t* iv)
{
  KeyExpansion(ctx->RoundKey, key);
  for(int i=0; i<AES_BLOCKLEN; i++)
    ctx->Iv[i]=iv[i];
...
void AES_CBC_encrypt_buffer(aes256_context* ctx, uint8_t* buf, size_t length)
{
  size_t i;
  uint8_t *Iv = ctx->Iv;
  for (i = 0; i < length; i += AES_BLOCKLEN)
