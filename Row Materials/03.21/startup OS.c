...
static void  EE_oo_autostart_alarms(AppModeType Mode)
{
  ...
  for ( t = 0U; t < n; t++ ) {
    EE_TYPEALARM alarm_temp = compute_alarm_id(EE_oo_autostart_alarm_data[Mode].alarm, t);
    EE_oo_handle_rel_counter_object_insertion(alarm_temp, EE_oo_autostart_alarm_increment[alarm_temp], EE_oo_autostart_alarm_cycle[alarm_temp]);
  }
}
...
static const EE_as_schedule_table_autostart_data *
  get_schedule_table_autostart_data ( const EE_as_schedule_table_autostart_data scheduletable_data[], EE_UREG t )
{
...
static void  EE_oo_autostart_schedule_tables ( AppModeType Mode )
{
  ...
  for ( t = 0U; t<n; t++ ) {
    const EE_as_schedule_table_autostart_data * sched_data_ref = get_schedule_table_autostart_data(EE_as_schedule_table_autostart[Mode].autostart_schedule_tables, t);
